...
(function () {
    ...
    Markdown.Converter = function (OPTIONS) {
            ...
            */
            ...
            text = text.replace(/^(\#{1,6})[ \t]*(.+?)[ \t]*\#*\n+/gm,
                function (wholeMatch, m1, m2) {
                    ...
                    // turn text into ids
                    const textid = m2.replace(/[^\w\-]/g, "-").toLowerCase();
                    return "<h" + h_level + " id=\"" + textid + "\">" + _RunSpanGamut(m2) + "</h" + h_level + ">\n\n";
                }
            );
        ...
        function _DoCodeBlocks(text) {
            ...
            text = text.replace(/(?:\n\n|^\n?)```(\w+)?\n([\s\S]*?)\n```(\n*[ ]{0,3}[^ \t\n]|(?=~0))/g,
                function (wholeMatch, lang, codeblock, nextChar) {
                    codeblock = _EncodeCode(_Outdent(codeblock));
                    codeblock = _Detab(codeblock);
                    codeblock = codeblock.replace(/^\n+/g, ""); // trim leading newlines
                    codeblock = codeblock.replace(/\n+$/g, ""); // trim trailing whitespace
            ...
            */
            ...
            text = text.replace(/(^|[^\\`])(`+)(?!`)([^\r]*?[^`])\2(?!`)/gm,
                function (wholeMatch, m1, m2, m3, m4) {
                    ...
                    return m1 + "<div class=\"code-div\"><code>" + c + "</code></div>";
                }
            );
